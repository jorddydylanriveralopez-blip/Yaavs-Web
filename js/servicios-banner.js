(function () {
  const section = document.querySelector(".servicios-hero-banner");
  const host = document.querySelector("[data-servicios-banner-media]");
  if (!section || !host) return;

  const cfg = window.YAAVS_SERVICIOS_BANNER || {};
  const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  const intervalMs = cfg.interval || 5500;
